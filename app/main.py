...
def main(page: ft.Page):
    ...
    source_comp_dd = ft.Dropdown(label="Empresa", width=300, on_change=lambda e: on_source_comp_change(e.data, cargo_dd))
    dest_city_dd = ft.Dropdown(label="Destino", width=300, on_change=lambda e: update_comp_dd(e.data, dest_comp_dd, cargo_dd))
    ...
    trailer_dd = ft.Dropdown(label="Reboque", width=300, value="curtain", options=[ft.dropdown.Option("curtain", "Sider"), ft.dropdown.Option("flatbed", "Prancha"), ft.dropdown.Option("refrigerated", "Frigorífico")])

    own_source_city_dd = ft.Dropdown(label="Origem", width=300, on_change=lambda e: update_comp_dd(e.data, own_source_comp_dd, own_cargo_dd))
    own_source_comp_dd = ft.Dropdown(label="Empresa", width=300, on_change=lambda e: on_source_comp_change(e.data, own_cargo_dd))
    own_dest_city_dd = ft.Dropdown(label="Destino", width=300, on_change=lambda e: update_comp_dd(e.data, own_dest_comp_dd, own_cargo_dd))
    ...
    mercado_fretes_view = ft.Column([
        ft.Row([create_section("📍 ORIGEM", ft.Column([source_city_dd, source_comp_dd])), create_section("🏁 DESTINO", ft.Column([dest_city_dd, dest_comp_dd]))], spacing=20),
        dist_text,
        create_section("📦 CARGA E REBOQUE", ft.Row([cargo_dd, trailer_dd], spacing=20)),
        ft.ElevatedButton("GERAR NO MERCADO DE FRETES", bgcolor=SUCCESS_COLOR, color="white", height=60, width=940)
    ], spacing=20)

    mercado_cargas_view = ft.Column([
        ft.Row([create_section("📍 ONDE VOCÊ ESTÁ", ft.Column([own_source_city_dd, own_source_comp_dd])), create_section("🏁 PARA ONDE VAI", ft.Column([own_dest_city_dd, own_dest_comp_dd]))], spacing=20),
        own_dist_text,
        create_section("📦 SELECIONE A CARGA", ft.Row([own_cargo_dd], spacing=20)),
        ft.ElevatedButton("GERAR PARA MEU REBOQUE", bgcolor="#2196F3", color="white", height=60, width=940)
    ], spacing=20)
